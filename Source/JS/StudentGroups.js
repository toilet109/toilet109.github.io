var StudentGroups = new Object();

StudentGroups.data = {
    SelectID    : 0,
    WhichIs     : `https://www.google.com.tw/maps/place/%E5%9C%8B%E7%AB%8B%E8%87%BA%E5%8D%97%E5%A4%A7%E5%AD%B8%E6%85%B6%E5%BF%A0%E8%A1%97%E8%97%9D%E8%A1%93%E7%89%B9%E5%8D%80%E7%AC%AC%E4%B8%89%E7%A9%BA%E9%96%93/@22.98498,120.2052822,18z/data=!4m8!1m2!2m1!1z6Ie65Y2X5aSn5a246Jed6KGT54m55Y2A!3m4!1s0x0:0x791dc80901c11908!8m2!3d22.9850923!4d120.2057954`, //地點
    WhyCall     : `mailto:nutnvad@gmail.com`, //聯絡方式
    FaceBookLink: `http://www.facebook.com/nutnvad`,
    GroupsData:[
        {
            Name             : `鏤空茶具`, //作品名稱
            Type             : `ThreeDimensionalModeling`, //作品種類
            About            : `倪田Ni,Tian`, //小組介紹
            ProjectMeta      : `使用鏤空雕刻手法在不改變太多茶器傳統的造型下給予器具更多的裝飾可能性；因為鏤空手法會破壞坯體影響實用性，所以藉由分離與接合兩層土壁達到保留的目的；蜂窩圖騰有著傳統的堆疊穩固與現代的直線幾何圖形融合，象徵著時代的交合與轉變，所以選擇這個圖騰來裝飾傳統茶具，以刻劃線條呈現出點、面、光影與觸覺的變化給予茶器造型上立體的裝飾。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/鏤空茶具/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/鏤空茶具/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `倪田Ni,Tian`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/鏤空茶具/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/鏤空茶具/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/鏤空茶具/2.jpg`,
            ],
        },
        {
            Name             : `Woong Kitchen`, //作品名稱
            Type             : `PackagingDesign`, //作品種類
            About            : `黃泰雄 Hwang,Tae-Woong`, //小組介紹
            ProjectMeta      : `我來自韓國，創作作品是將韓國和台灣的飲食文化，互相融合創造出新穎的創意料理，為了讓大家一起動手做，我透過AR將料理的食材和煮菜的過程，製作成3部影片呈現給大家。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/Woong Kitchen/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/Woong Kitchen/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `黃泰雄 Hwang,Tae-Woong`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/Woong Kitchen/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                `https://www.youtube.com/watch_popup?v=BVVEccqKI00;vq=hd1080&amp;version=3`,
                `https://www.youtube.com/watch_popup?v=F3a4rU4-ZlI;vq=hd1080&amp;version=3`,
                `https://www.youtube.com/watch_popup?v=hYmyUAkW3iA;vq=hd1080&amp;version=3`,
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/Woong Kitchen/1.jpg`,
            ],
        },
        {
            Name             : `揭露-人民的覺醒`, //作品名稱
            Type             : `PackagingDesign`, //作品種類
            About            : `吳昌胤 Wu,Chang-Yin`, //小組介紹
            ProjectMeta      : `浮屍紙條內容強調言論自由的可貴，在一些國家和地區是沒有此項權利的，透過此作品我想要讓人們更重視和瞭解其的重要性。\n\n我以『水』為媒介，當紙條碰到水便能顯現底下的裸身人形，透過表層的特殊油墨及藉著水消筆寫下秘密傳遞，完成碰水秘密筆跡消失人形浮現的過程; 議題明信片選材於台灣社會上的議題，明信片在傳遞的同時也警醒著人們並對議題加以注意和進行反思，透過Instagram的發佈且加以說明，讓此作品持續影響並融入於大眾日常生活當中。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/揭露-人民的覺醒/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/揭露-人民的覺醒/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `吳昌胤 Wu,Chang-Yin`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/揭露-人民的覺醒/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/揭露-人民的覺醒/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/揭露-人民的覺醒/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/揭露-人民的覺醒/3.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/揭露-人民的覺醒/4.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/揭露-人民的覺醒/5.jpg`,
            ],
        },
        {
            Name             : `日日藥時`, //作品名稱
            Type             : `PackagingDesign`, //作品種類
            About            : `鄧如瑄 Teng,Ru-Shuan`, //小組介紹
            ProjectMeta      : `今天您吃了嗎？\n\n現在慢性病已經涉及到許多年齡層，就算沒有疾病也會食用保健食品來保養自己身體健康，現今吃藥的行為已經成為現代社會的常態。而在高年齡層的人不免患於慢性病，藉由每天都必須吃藥的狀態，將藥盒與時鐘結合，用最普及的物件提醒三餐吃藥的重要。而火鍋料已成為家中儲備名單，幾乎餐餐都能為佳餚增添風味卻往往是三高患者的甜蜜毒藥，希望患者能充分了解這些食物成分並慎選食品和提醒吃藥。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/日日藥時/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/日日藥時/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `鄧如瑄 Teng,Ru-Shuan`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/日日藥時/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/日日藥時/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/日日藥時/2.jpg`,
            ],
        },
        {
            Name             : `補形湯`, //作品名稱
            Type             : `PackagingDesign`, //作品種類
            About            : `陳恩嘉 Chen,En-Chia`, //小組介紹
            ProjectMeta      : `補形湯為以形補形為概念出發的茶飲。\n\n人們對以食補身有極大的迷思也因此傷害了許多物種，在這裡選用植物性有相同功能的民間配方去制成這些動物的形體，達到喝下去補身同時也有心安的感受。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/補形湯/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/補形湯/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `陳恩嘉 Chen,En-Chia`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/補形湯/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/補形湯/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/補形湯/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/補形湯/3.jpg`,
            ],
        },

        {
            Name             : `太陽夜行: The sun travels in the night`, //作品名稱
            Type             : `PictureBooks`, //作品種類
            About            : `吳芮甄 Wu,Jui-Chen`, //小組介紹
            ProjectMeta      : `太陽睡不著,想出去玩,但夜晚中誰醒著呢？\n\n夜行性動物們現身於城市中,帶著太陽來了一場旅程。\n\n此故事以三元素構成：森林立板、城市書面、象徵太陽的手電筒。希望讀者以太陽的身分去探索夜晚奇獸，同時思考生態危機。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/太陽夜行: The sun travels in the night/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/太陽夜行: The sun travels in the night/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `吳芮甄 Wu,Jui-Chen`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/太陽夜行: The sun travels in the night/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/太陽夜行: The sun travels in the night/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/太陽夜行: The sun travels in the night/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/太陽夜行: The sun travels in the night/3.jpg`,
            ],
        },
        {
            Name             : `短髮村`, //作品名稱
            Type             : `PictureBooks`, //作品種類
            About            : `鄭語荷 Jheng,Yu-He`, //小組介紹
            ProjectMeta      : `長頭髮的少女來到了短髮薑餅人的村莊，從一開始覺得自己是很突兀的存在，到後來慢慢學會接受自己，也練習為自己的生活做決定。\n\n這是一個系列故事的一部分，背景設定是在一個萬物皆有靈的世界，每一個物品以及動植物都有自己的想法，可以決定自己的樣子和他們在世界上扮演的角色。\n\n長髮女孩就是在這樣的環境裡逐漸去思考生命的價值和意義，學會觀察周遭並反思什麼才是自己真正想要的。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/短髮村/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/短髮村/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `鄭語荷 Jheng,Yu-He`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/短髮村/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/短髮村/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/短髮村/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/短髮村/3.jpg`,
            ],
        },
        {
            Name             : `日頭`, //作品名稱
            Type             : `Album`, //作品種類
            About            : `賴奕翔 Lai,Yi-Xiang`, //小組介紹
            ProjectMeta      : `日頭是我的樂團-噪鬱部長 創團後寫的第一首歌 歌曲內容與包裝皆以日出象徵黑夜終將過去，痛苦的過程都將轉變為明天更好的自己。包裝以太陽樣式與光影做出日出的感覺。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/日頭/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/日頭/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `賴奕翔 Lai,Yi-Xiang`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/日頭/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/日頭/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/日頭/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/日頭/3.jpg`,
            ],
        },
        {
            Name             : `Tear pot`, //作品名稱
            Type             : `Animation`, //作品種類
            About            : `馬晨玉 Ma,Chen-Yu`, //小組介紹
            ProjectMeta      : `故事中出現的眼球都是小鹿們的眼睛，小鹿在此故事中代表的是有經濟價值的自然資源，眼球們被製成各種美好夢幻的商品，象徵人類藉由對產品的包裝，使人們活在自己塑造的美好假象中、而對於剝削自然以及生活中造成的殺戮逐漸麻木。`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/Tear pot/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/Tear pot/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `馬晨玉 Ma,Chen-Yu`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/Tear pot/1.jpg`,
            ],
            ProjectMoviePath : [   //作品影片路徑
                `https://www.youtube.com/embed/aRYFeUXQlwc`,
                `https://www.youtube.com/embed/IGqxoGGcwDI`,
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/Tear pot/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/Tear pot/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/Tear pot/3.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/Tear pot/4.jpg`,
            ],
        },
        {
            Name             : `生產斂 Blindness`, //作品名稱
            Type             : `Animation`, //作品種類
            About            : `鐘元真 Chung,Yuan-Zhen`, //小組介紹
            ProjectMeta      : `兩家競爭的工廠 類似的商品各有代表自身的擁護者。\n\n然而塑膠皮下的內容物，似乎不是買家所在意的。\n不管裡面有多少問題或其實並不適合拿來製成產品， 只要是他們所支持的店家,就無條件買單。\n\n深怕被對手贏過。 一批一批的貨物持續的賣著,有人暗地使出陰招想贏得勝利。 \n\n最終被有自我意識的民眾發現並擊殺，最後留下一地鮮血。\n\n但其實真正的凶手卻另有其人，被殺者即使獲利，依然只是個被利用的棋子。\n\n`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/生產斂 Blindness/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/生產斂 Blindness/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `鐘元真 Chung,Yuan-Zhen`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/生產斂 Blindness/1.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                `https://www.youtube.com/embed/7YL6OLRzvno`,
                `https://www.youtube.com/embed/Ot19ZjdTSc0`,
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/生產斂 Blindness/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/生產斂 Blindness/2.jpg`,
            ],
        },
        {
            Name             : `魚月島 The island of the fish and moon`, //作品名稱
            Type             : `Animation`, //作品種類
            About            : `許家榕 Syu,Jia-Rong\n 陳岳峰 Chen,Yue-Feng\n 黃靖博 Huang,Ching-Po`, //小組介紹
            ProjectMeta      : `為什麼度假島上的海灘如此美麗，卻禁止嬉水？科學家老頓在度假村的沙灘上做研究時意外落海，發現了為了維持度假島美麗的風光，人們將垃圾丟進魚群的棲所，海中的生物開始變異，垃人類在海上建了樂園，卻在海底下築起一座垃圾城市，垃圾城最終會以什麼方式打碎人類以為的樂園？\n\nWhy do they forbid people not to play with the water in the beautiful island？In the resort, scientist Oldton falls into the sea when he is doing some research. He finds that people not only built up a paradise but also built a garbage city under the sea.Will the paradise collapse？`, //作品介紹
            ImagePhotoPath   : `./Source/IMG/StudentGroupsImagePhoto/魚月島 The island of the fish and moon/1.jpg`, //形象照路徑
            PosterPhotoPath  : `./Source/IMG/StudentGroupsPoster/魚月島 The island of the fish and moon/1.png`, //海報照路徑
            MumberName       : [   //組員名稱
                `許家榕 Syu,Jia-Rong`,
                `陳岳峰 Chen,Yue-Feng`,
                `黃靖博 Huang,Ching-Po`,
            ],
            MumberPhotoPath  : [
                `./Source/IMG/StudentGroupsMumberPhoto/魚月島 The island of the fish and moon/1.jpg`,
                `./Source/IMG/StudentGroupsMumberPhoto/魚月島 The island of the fish and moon/2.jpg`,
                `./Source/IMG/StudentGroupsMumberPhoto/魚月島 The island of the fish and moon/3.jpg`,
            ],
            ProjectMoviePath  : [   //作品影片路徑
                `https://www.youtube.com/embed/N8YM-Ga755Q`,
            ],
            ProjectPhotoPath : [   //作品照片路徑
                `./Source/IMG/StudentGroupsProjectPhoto/魚月島 The island of the fish and moon/1.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/魚月島 The island of the fish and moon/2.jpg`,
                `./Source/IMG/StudentGroupsProjectPhoto/魚月島 The island of the fish and moon/3.jpg`,
            ],
        },
    ]
};


StudentGroups.event = {
    //切換下一組群組的資料 + 回傳索引
    GetNextGroup:function(){
        StudentGroups.data.SelectID++;
        if(StudentGroups.data.SelectID === StudentGroups.data.GroupsData.length){
            return StudentGroups.data.SelectID = 0;
        }
        return StudentGroups.data.SelectID
    },
    //切換上一組群組的資料 + 回傳索引
    GetBeforeGroup:function(){
        StudentGroups.data.SelectID--;
        if(StudentGroups.data.SelectID === -1){
            StudentGroups.data.SelectID = StudentGroups.data.GroupsData.length-1;
        }
        return StudentGroups.data.SelectID
    },
    //取得目前對應ID
    GetNowSelectID:function() {
        return StudentGroups.data.SelectID;
    },
    SetNowSelectID:function(SetID) {
        StudentGroups.data.SelectID = SetID;
    }
}